"""
2D rendering framework copied form openai gym classic_control/rendering
"""
# pylint: skip-file
# flake8: noqa
import os
...
def get_window(width, height, display, **kwargs):
    ...
    return pyglet.window.Window(width=width, height=height, display=display, config=config,
...
class Viewer(object):
    ...
    def close(self):
        if self.isopen and sys.meta_path:
            # ^^^ check sys.meta_path to avoid 'ImportError:
    ...
    def render(self, return_rgb_array=False, save_images=False, path="../video"):
...
class SimpleImageViewer(object):
    ...
    def imshow(self, arr):
        if self.window is None:
            ...
            self.window = get_window(width=width, height=height, display=self.display, vsync=False,
                                     resizable=True)
            self.width = width
            self.height = height
            self.isopen = True
